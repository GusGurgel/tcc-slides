...
\begin{frame}{Escopo Geral do Problema}
    \begin{itemize}
        ...
        \item<6-> \textbf{Problema:} Em geral, a geração desses conteúdos é guiada através de
...
\begin{frame}{Solução: PCG + LLMs}
    \begin{itemize}
        ...
        \item <3-> Em vez de inserir valores numéricos, o utilizador descreve o
              conteúdo a ser gerado utilizando linguagem natural.
    \end{itemize}
\end{frame}
...
\begin{frame}{Objetivos}
    ...
    \begin{itemize}
        \item<2-> Desenvolver um pipeline de geração que receba uma descrição
              textual e produza uma representação de mapa estruturada.
        ...
        \begin{column}{0.5\textwidth}
            \begin{itemize}
                \item Em 1980 surge \textbf{Rogue}, um jogo de terminal onde o
                      ...
                      vida.
                \item<2-> Esse jogo foi tão marcante que deu origem a um gênero
                      inteiro chamado Roguelike, que se baseia em elementos como:
                      \begin{itemize}
                          \item <3-> Geração procedural.
                          \item <4-> Permadeath (morte permanente).
                          \item <5-> Exploração estratégica.
                          \item <6-> Alta rejogabilidade.
                      \end{itemize}
            \end{itemize}
        \end{column}
    \end{columns}
...
\begin{frame}{Implementação do Gerador de Assets}
    \begin{itemize}
        ...
        \item<2-> \textbf{Solução}: Utilização da técnica de \textit{Structured
                  Outputs}.  Uso modelos ajustados para responder seguindo um esquema
              JSON.
    \end{itemize}
    \vspace{0.5cm}
    \begin{itemize}
        \item<3-> \textbf{Problema 2}: O LLM gera saídas textuais, porém o jogo precisa
              de ativos visuais para representar items, inimigos e ambientes.
        \item<4-> \textbf{Solução}: Utilização da técnica de \textit{RAG}. Um banco
...
\begin{frame}{Conclusão e Contribuições}
    \begin{itemize}
        ...
        \item<3-> \textbf{Coerência e Eficiência Semântica}: Através das
...
\begin{frame}{Limitações e Trabalhos Futuros}
    \begin{itemize}
        \item<1-> \textbf{Limitação 1}: Os bancos vetoriais tem sua expressividade
              limitada pela quantidade de tiles disponíveis.
        \item<2-> \textbf{Possível Solução}: Modelos de difusão (como Stable
              Diffusion) para gerar texturas em tempo real
              \vspace{0.2cm}
        \item<3-> \textbf{Limitação 2}: O sistema apenas gera objetos unitários
              isolados.
        \item<4-> \textbf{Possível Solução}: Evoluir geração de conjuntos de
              elementos, permitindo a construção de micro-cenários mais detalhados e
              coesos.
              \vspace{0.2cm}
        \item<5-> \textbf{Limitação 3}: O projeto é focado apenas na geração de
              mapas para jogos Roguelikes.
        \item<6-> \textbf{Possível Solução}: Adaptar o sistema para outros tipos

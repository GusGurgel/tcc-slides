...
\begin{frame}{Geração de texto com LLMs}
    \begin{columns}{}
        ...
        \begin{column}{0.4\textwidth}
            ...
            uma quantidade \textbf{extremamente grande de texto, geralmente
                adquirido da internet}.
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Roguelike}
    ...
    desenvolvido um jogo \textit{Roguelike} capaz de processar um
    \textit{AssetBundle} em formato JSON e transformá-lo em um mapa jogável.
\end{frame}
...
%     \begin{figure}
%         \centering
%         \caption{Sistema de Campo de Visão}
%         \includegraphics[height=5cm]{images/field_of_view.png}
%         \source{Elaborado pelo autor}
%     \end{figure}
% \end{frame}
...
\begin{frame}{Conclusão e Contribuições}
    \begin{itemize}
        ...
        \item<2-> \textbf{Mitigação de Alucinações via RAG}: A implementação de
              Retrieval-Augmented Generation (RAG) com saídas estruturadas e um banco
              vetorial segmentado (Entidades, Itens, Ambientes) garantiu a coerência
              visual e temática do jogo.
              \begin{itemize}
                  \item <2-> \textbf{Contribuição de Dados}: O trabalho entrega um
                        dataset relevante de 489 pares de descrições e imagens (pixel art),
                        estabelecendo uma ponte técnica sólida entre linguagem natural e
                        ativos visuais.
              \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Conclusão e Contribuições}
    \begin{itemize}
        \item<1-> \textbf{Viabilidade Técnica Local}: Os testes mostraram que
              ...
              comparável a modelos gigantes (120b), provando que a tecnologia é
              acessível em hardware de consumo, sem necessidade de nuvem.
        \item<2-> \textbf{Eficiência Semântica e Coerência}: Através da métrica de
              Reconstrução Semântica e de Coerência, comprovou-se que o sistema
              preserva fielmente a intenção do usuário ao transformar texto em
              elementos de jogo funcionais.
        \item<3-> \textbf{Democratização do Desenvolvimento}: A integração entre
              PCG clássico e IA Generativa reduz barreiras técnicas, permitindo a
              criação de experiências de jogo customizadas e coerentes através de
              linguagem natural.
    \end{itemize}
\end{frame}
...
\begin{frame}{Limitações e Trabalhos Futuros}
    \begin{itemize}
        \item<1-> \textbf{Texturas Dinâmicas com IA}: Substituir o banco de imagens
              fixo por modelos de difusão (como Stable Diffusion) para gerar texturas
              em tempo real, tornando o visual tão ilimitado quanto o texto.
        \item<2-> \textbf{Cenários Complexos}: Evoluir a criação de objetos isolados
              para a geração de conjuntos de elementos (prefabs), permitindo a
              construção de micro-cenários mais detalhados e coesos.
        \item<3-> \textbf{Expansão de Gêneros e Narrativa}: Adaptar o sistema para
              outros tipos de jogos (como RPGs), incluindo a criação dinâmica de NPCs,
              diálogos e missões personalizadas.
    \end{itemize}
\end{frame}
